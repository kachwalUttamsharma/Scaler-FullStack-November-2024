...
const refreshBishop = () => {
  if (prevBishop) {
    ...
    let minLeft = Math.min(prevBishop.row, prevBishop.col),
      minRight = Math.min(prevBishop.row, 7 - prevBishop.col);
    let prev_r1 = prevBishop.row - minLeft,
      prev_c1 = prevBishop.col - minLeft;
    let prev_r2 = prevBishop.row - minRight,
      prev_c2 = prevBishop.col + minRight;
...
board.addEventListener("click", (e) => {
  let row = int(e.target.parentNode.id),
    col = int(e.target.id);
  const bishop = () => {
    prevBishop = { row: row, col: col };
    let minLeft = Math.min(row, col),
      minRight = Math.min(row, 7 - col);
    let r1 = row - minLeft,
      c1 = col - minLeft;
    let r2 = row - minRight,
      c2 = col + minRight;
